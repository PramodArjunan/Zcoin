	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException 
	 {
		 ...
		 try 
		 {
	            ...
	            System.out.println("--------------------------------");
	            System.out.println("Deposit"+UserCreation.getDeposit());
	            System.out.println("Withdraw"+UserCreation.getWithdraw());
	            System.out.println("zid"+UserCreation.getZidAmount());
	            System.out.println("--------------------------------");
	            // check password is corect or not
	            if(Password.equals(UserCreation.getPassword()))
	            {
	            	if(UserCreation.getDeposit()!=0)
	            	{
	            		   
				           System.out.println("Entered");
				            int balance=UserCreation.getRc()+UserCreation.getDeposit();
				            Account.setRc(balance);
				          
				            String insert="insert into transactions(userzid,transaction,transactionamount,balance) values('"+UserCreation.getZid()+"','Deposit','"+UserCreation.getDeposit()+"','"+balance+"')";
				            mysql.mysqlInsert(insert);
				            Account.setDeposit(0);
				            
				            String update="update userinformation set rc='"+UserCreation.getRc()+"' where email='"+Email+"'";
	            	...
	            	else if(UserCreation.getZidAmount()!=0)
	            	{
			            ...
			            if(count==1)
			            {
		            		...
		            		else
		            		{
					            ...
					            String updatesenderrc="update userinformation set rc='"+UserCreation.getRc()+"' where email='"+Email+"'";
